// Vendored from TLGS tlgsutils/url_parser.cpp.
...
Url& Url::withHost(const std::string& value) {
    cache_.clear();
    host_ = value;
    return *this;
}
Url& Url::withPath(const std::string& value, bool normalize_path) {
    cache_.clear();
    path_ = (value.empty() || value.front() != '/') ? "/" + value : value;
    if (normalize_path)
        path_ = std::filesystem::path(path_).lexically_normal().generic_string();
    return *this;
}
Url& Url::withParam(const std::string& value) {
    cache_.clear();
    param_ = value;
    return *this;
}
Url& Url::withProtocol(const std::string& value) {
    cache_.clear();
    protocol_ = value;
    default_port_ = protocolDefaultPort(protocol_);
    return *this;
}
Url& Url::withPort(unsigned short value) {
    cache_.clear();
    port_ = value;
    return *this;
}
...
Url& Url::withDefaultPort(unsigned short value) {
    cache_.clear();
    default_port_ = value;
    return *this;
}
Url& Url::withFragment(const std::string& value) {
    cache_.clear();
    fragment_ = value;
    return *this;
}
...
int Url::port(int default_port) const {
    return port_ != 0 ? port_ : (default_port != 0 ? default_port : default_port_);
}
